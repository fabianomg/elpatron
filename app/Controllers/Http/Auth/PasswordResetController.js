...
class PasswordResetController {
  ...
  async sendResetLinkEmail({ request, session, response }) {
    ...
    const validation = await validate(request.only('email', 'username'), {
    ...
    try {
      ...
      await PasswordReset.query().where('username', user.username).delete()
      ...
      session.flash({
        notification: {
          type: 'danger',
          message: 'Desculpe, não há usuário com este username cadastrado.'
        }
      })
